...
public class CarnetImpl extends UnicastRemoteObject implements Carnet {
    ...
    private void executeCallbacks(Individu i) {
    	List<CallbackOnCreation> failed_callbacks =  new ArrayList<CallbackOnCreation>();
    	for(CallbackOnCreation cb: this.callbacks) {
    		try {
    			cb.eventCreated(i);
    		} catch (RemoteException e) {
    			// e ignorée
    			failed_callbacks.add(cb);
    		}
    	}
    	
    	this.callbacks.removeAll(failed_callbacks);
